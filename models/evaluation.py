...
def multi_label_metrics(true_label,pred_label):
    ...
    # fpr["micro"], tpr["micro"], _ = roc_curve(true_label.ravel(),
    #   pred_label.ravel())
    ...
    # print("Macro AUC: {0:0.2f}, Micro AUC: {1:0.2f}".format(roc_auc["macro"],
    #   roc_auc["micro"]))
    phen_auc = []
    for i in range(n_classes):
...
def print_classification_metrics(true_label,pred_label):
    ...
    (precisions, recalls, _) = precision_recall_curve(true_label, pred_label,
            pos_label=1)
...
def regression_metrics(true_label,pred_label,ts=None):
    ...
    pred_stay = []
    if ts is None:
        for i,(a,b) in enumerate(zip(np.squeeze(true_label),
