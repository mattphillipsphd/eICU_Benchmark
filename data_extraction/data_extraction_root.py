...
def data_extraction_root(args):
    ...
    utils.break_up_stays_by_unit_stay(patients, args.output_dir,stayid=stay_id,
            verbose=1)
    del patients

    # print("reading lab table")
    lab = utils.read_lab_table(args.eicu_dir)
    utils.break_up_lab_by_unit_stay(lab, args.output_dir, stayid=stay_id,
            verbose=1)
    del lab

    print("reading nurseCharting table, might take some time")
    nc = utils.read_nc_table(args.eicu_dir)
    utils.break_up_stays_by_unit_stay_nc(nc, args.output_dir, stayid=stay_id,
            verbose=1)
    del nc
...
def main():
    ...
    parser.add_argument('--eicu_dir', type=str,
            help="Path to root folder containing all the patietns data")
    parser.add_argument('--output_dir', type=str, help="Directory where the "\
            "created data should be stored.")
